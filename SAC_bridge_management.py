...
class environment():
    ...
    def step(self, bridge_infor, states, actions, hidden_state):
        ...
        new_state = new_state.astype(np.float32).reshape(1, len(new_state))
...
class QCritic_build(keras.Model):
    def __init__(self, state_size, action_size, batchnorm=True, hidden=[2048, 2048, 2048]):
        super(QCritic_build, self).__init__()
        self.fc1 = layers.Dense(hidden[0], input_shape=[None, state_size], kernel_regularizer=regularizers.l2(0.001))
        self.fc2 = layers.Dense(hidden[1], kernel_regularizer=regularizers.l2(0.001))
        self.fc3 = layers.Dense(hidden[2], kernel_regularizer=regularizers.l2(0.001))
        self.fc4 = layers.Dense(action_size, kernel_regularizer=regularizers.l2(0.001))  # Q(s,·)
        self.bn1 = layers.BatchNormalization()
        self.bn2 = layers.BatchNormalization()
        self.bn3 = layers.BatchNormalization()
        self.batchnorm = batchnorm
    ...
    def call(self, x):
        if self.batchnorm:
            x = tf.nn.relu(self.bn1(self.fc1(x)))
            res1 = x
            x = tf.nn.relu(self.bn2(self.fc2(x))) + res1
            res2 = x
            x = tf.nn.relu(self.bn3(self.fc3(x))) + res2
            return self.fc4(x)
        else:
            x = tf.nn.relu(self.fc1(x))
            res1 = x
            x = tf.nn.relu(self.fc2(x)) + res1
            res2 = x
            x = tf.nn.relu(self.fc3(x)) + res2
            return self.fc4(x)

def get_action(Actor_network, state, greedy=False):
    # state: shape (1, state_dim)
    logits = Actor_network(state)  # (1, n_actions)
    log_probs = tf.nn.log_softmax(logits)  # (1, n_actions)
    probs = tf.nn.softmax(logits)          # (1, n_actions)

    if greedy:
        action = int(tf.argmax(probs[0]).numpy())
    else:
        action = int(np.random.choice(probs.shape[1], p=probs[0].numpy()))

    logp = float(log_probs[0, action].numpy())
    return action, logp

class ReplayBuffer:
    def __init__(self, state_dim, capacity=200000):
        ...
        self.sp = np.zeros((capacity, state_dim), np.float32)
...
class Agent():
    def __init__(self,
        ...
        # 用 legacy optimizers，且 Q1/Q2 分开，避免 “optimizer cannot recognize variable”
        self.q1_optimizer = tf.keras.optimizers.legacy.Adam(5e-5)
        self.q2_optimizer = tf.keras.optimizers.legacy.Adam(5e-5)
        self.actor_optimizer = tf.keras.optimizers.legacy.Adam(5e-6)

        # alpha 自动调：优化 log_alpha
        self.log_alpha = tf.Variable(0.0, dtype=tf.float32, trainable=True)
        self.alpha_optimizer = tf.keras.optimizers.legacy.Adam(5e-5)

        # 离散动作目标熵（常用 -log(A)）
        self.target_entropy = -float(np.log(self.n_actions))
    ...
    def save(self, folder_name, **kwargs):
        ...
        self.Q1.save_weights(folder_name + '/Q1.ckpt')
        self.Q2.save_weights(folder_name + '/Q2.ckpt')
        self.Q1_target.save_weights(folder_name + '/Q1_target.ckpt')
        self.Q2_target.save_weights(folder_name + '/Q2_target.ckpt')
        np.save(os.path.join(folder_name, "log_alpha.npy"), self.log_alpha.numpy())

    def load(self, folder_name, **kwargs):
        self.Actor.load_weights(folder_name + '/Actor.ckpt')
        self.Q1.load_weights(folder_name + '/Q1.ckpt')
        self.Q2.load_weights(folder_name + '/Q2.ckpt')
        self.Q1_target.load_weights(folder_name + '/Q1_target.ckpt')
        self.Q2_target.load_weights(folder_name + '/Q2_target.ckpt')
        p = os.path.join(folder_name, "log_alpha.npy")
        if os.path.exists(p):
            self.log_alpha.assign(np.load(p).astype(np.float32))
    ...
    def _soft_update(self, net, target):
        ...
        for wi, wti in zip(w, wt):
            new_wt.append(self.tau * wi + (1.0 - self.tau) * wti)
        target.set_weights(new_wt)
    ...
    def sac_update(self, replay_buffer, batch_size=256):
        s, a, r, sp, done = replay_buffer.sample(batch_size)
        ...
        # ---------------- Critic update ----------------
        # 先算 target y（不需要梯度）
        logits_sp = self.Actor(sp)
        log_pi_sp = tf.nn.log_softmax(logits_sp)    # (B, A)
        pi_sp = tf.nn.softmax(logits_sp)            # (B, A)

        q1_t = self.Q1_target(sp)                   # (B, A)
        q2_t = self.Q2_target(sp)                   # (B, A)
        qmin_t = tf.minimum(q1_t, q2_t)

        v_sp = tf.reduce_sum(pi_sp * (qmin_t - self.alpha * log_pi_sp), axis=1)  # (B,)
        y = r + self.gamma * (1.0 - done) * v_sp                                 # (B,)

        # Q1
        with tf.GradientTape() as tape_q1:
            q1 = self.Q1(s)  # (B, A)
            q1_sa = tf.gather(q1, a[:, None], batch_dims=1)[:, 0]
            loss_q1 = tf.reduce_mean(tf.square(y - q1_sa))
        grads_q1 = tape_q1.gradient(loss_q1, self.Q1.trainable_variables)
        self.q1_optimizer.apply_gradients(zip(grads_q1, self.Q1.trainable_variables))

        # Q2
        with tf.GradientTape() as tape_q2:
            q2 = self.Q2(s)  # (B, A)
            q2_sa = tf.gather(q2, a[:, None], batch_dims=1)[:, 0]
            loss_q2 = tf.reduce_mean(tf.square(y - q2_sa))
        grads_q2 = tape_q2.gradient(loss_q2, self.Q2.trainable_variables)
        self.q2_optimizer.apply_gradients(zip(grads_q2, self.Q2.trainable_variables))
        ...
        with tf.GradientTape() as tape_pi:
            logits = self.Actor(s)
            log_pi = tf.nn.log_softmax(logits)      # (B, A)
            pi = tf.nn.softmax(logits)              # (B, A)

            qmin = tf.minimum(self.Q1(s), self.Q2(s))  # (B, A)

            # minimize E[ sum pi * (alpha*log_pi - Q) ]
            loss_pi = tf.reduce_mean(tf.reduce_sum(pi * (self.alpha * log_pi - qmin), axis=1))

        grads_pi = tape_pi.gradient(loss_pi, self.Actor.trainable_variables)
        self.actor_optimizer.apply_gradients(zip(grads_pi, self.Actor.trainable_variables))
        ...
        with tf.GradientTape() as tape_alpha:
            ...
            entropy = -tf.reduce_mean(tf.reduce_sum(pi * log_pi, axis=1))  # scalar

            # push entropy towards target_entropy
            loss_alpha = -(self.log_alpha * (self.target_entropy + entropy))

        grad_alpha = tape_alpha.gradient(loss_alpha, [self.log_alpha])
        self.alpha_optimizer.apply_gradients(zip(grad_alpha, [self.log_alpha]))

        # ---------------- Soft update targets ----------------
        self._soft_update(self.Q1, self.Q1_target)
        self._soft_update(self.Q2, self.Q2_target)

        return float(loss_q.numpy()), float(loss_pi.numpy()), float(entropy.numpy()), float(self.alpha.numpy())
...
def main():
    ...
    # arch, 3 components, 12 preventive years
    ...
    transverse_initial_time = np.array([1] * 20)
    ...
    # Bridge information (固定参数)
    Bridge_information = np.array(
        [132 * 8 * 2, 10 * 4, 132 * 2 * 3, 132 * 4, (132 / 12 + 1) * 2, 8 * 4, 145.16 * 7, 10.7 * 3.17, 132, 10,
         6 * 10, 4.79, 9.04, 12.79, 16.05, 18.84, 21.18, 23.07, 24.53, 25.57, 26.19, 26.4,
    ...
    Actor_network = Actor_build(state_dim, n_actions)
    Q1 = QCritic_build(state_dim, n_actions)
    Q2 = QCritic_build(state_dim, n_actions)
    Q1_t = QCritic_build(state_dim, n_actions)
    Q2_t = QCritic_build(state_dim, n_actions)

    Environment = environment()
    agent = Agent(
        Actor_network, Q1, Q2, Q1_t, Q2_t, Environment,
        n_actions=n_actions, input_shape=state_dim, gamma=0.99, tau=0.005
    )

    replay = ReplayBuffer(state_dim=state_dim, capacity=200000)

    # ---------------- Training hyperparameters (SAC, faster schedule) ----------------
    # 每个 episode 100 步；你希望训练约 3 天，这里按“时间控制”而不是固定 episode 数。
    max_over_step = 100
    ...
    update_every = 8
    updates_per_update = 2  # 想更快可设 1；想更强可设 4（会更慢）

    # 3 天左右：用 wall-clock 控制（更可靠），到时自动停止
    max_wall_seconds = 3 * 24 * 3600  # 3 days

    # 也加一个最大总步数兜底（防止极端情况跑太久）
    ...
    csv_path = os.path.join(SCRIPT_DIR, "cumulative_reward.csv")
    if not os.path.exists(csv_path):
        with open(csv_path, "w") as f:
            f.write("episode,cumulative_reward,total_steps,alpha,entropy\n")

    ckpt_dir = os.path.join(SCRIPT_DIR, "checkpoints_window_best")
    os.makedirs(ckpt_dir, exist_ok=True)

    # 窗口不要太大，否则 3 天内保存次数太少；也不要太小，否则噪声大
    window_size = 1000
    ...
    while True:
        ...
        if (time.time() - start_time) >= max_wall_seconds:
            print("[STOP] Reached max wall time (3 days).")
            break
        ...
        ep_return = 0.0
        last_entropy = 0.0
        last_alpha = float(agent.alpha.numpy())
        last_loss_q = 0.0
        last_loss_pi = 0.0

        for t in range(max_over_step):
            a = agent.act(s, greedy=False)

            sp, cost, hp, _, _ = Environment.step(Bridge_information, s, a, h)
            r = float(cost[0])

            done = 1.0 if (t == max_over_step - 1) else 0.0
            replay.add(s.reshape(-1), a, r, sp.reshape(-1), done)
            ...
            # update schedule: every 8 env steps do 2 gradient updates
            if replay.size >= warmup_steps and (total_steps % update_every == 0):
                for _ in range(updates_per_update):
                    last_loss_q, last_loss_pi, last_entropy, last_alpha = agent.sac_update(
                        replay, batch_size=batch_size
                    )

            # time stop inside episode
            if (time.time() - start_time) >= max_wall_seconds or total_steps >= max_total_steps:
                break

        reward_sum = float(ep_return)
        cur_window_rewards.append(reward_sum)

        elapsed = time.time() - start_time
        print(
            "epoch num:", ep,
            " time stemp:", max_over_step,
            " Reward:", reward_sum,
            " alpha:", last_alpha,
            " entropy:", last_entropy,
            " total_steps:", total_steps,
            " elapsed(h):", round(elapsed / 3600.0, 3)
        )

        # CSV
        if ep % 100 == 0:
            with open(csv_path, "a") as f:
                f.write(f"{ep},{reward_sum},{total_steps},{last_alpha},{last_entropy}\n")

        # Plot (避免 figure 警告：不要重复传 figsize)
        plt.figure(1)
        plt.clf()
        t_plot.append(ep)
        ...
        plt.ylabel("cumulative reward", fontsize=8)

        if ep % 10 == 0:
            plt.savefig(os.path.join(PLOT_DIR, "reward_latest.png"), dpi=150, bbox_inches="tight")
        if ep % 100 == 0:
            plt.savefig(os.path.join(PLOT_DIR, f"reward_ep_{ep}.png"), dpi=150, bbox_inches="tight")

        # Window-best checkpoint
        if (ep + 1) % window_size == 0:
            cur_window_avg = float(np.mean(cur_window_rewards))
            if prev_window_avg is None:
                print(f"[WINDOW] ep {ep - window_size + 1}~{ep}: avg={cur_window_avg:.6f} (no previous window)")
                # 第一个窗口也存一份，防止中途断电/重启丢失
                agent.save(ckpt_dir)
                print(f"[CKPT] Saved initial window model to {ckpt_dir}")
            else:
                print(f"[WINDOW] ep {ep - window_size + 1}~{ep}: avg={cur_window_avg:.6f} vs prev_avg={prev_window_avg:.6f}")
                if cur_window_avg > prev_window_avg:
                    agent.save(ckpt_dir)
                    print(f"[CKPT] Saved window-best model to {ckpt_dir} (avg improved)")

            prev_window_avg = cur_window_avg
            cur_window_rewards = []
