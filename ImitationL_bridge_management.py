...
def Maintenance_list(deck_M_utility, superstructure_M_utility, substructure_M_utility,
    ...
    maintenance_cost_maintenance = np.zeros((15))
    ...
    if action_index[-1] < len(necessary_utility_index):
        ...
        for i in range(len(maintenance_necessary_list)):
            maintenance_cost_maintenance[maintenance_necessary_list[i]] = necessary_cost[maintenance_necessary_list[i]]
        ...
        if action_index[-1] - len(necessary_utility_index) <= len(optional_utility_index):
            ...
            maintenance_cost_maintenance = maintenance_cost_maintenance + necessary_cost
            ...
            for i in range(action_index[-1] - len(necessary_utility_index)):
                if optional_utility_index[ranking_optional_maintenance[i]] > 14 and optional_utility_index[ranking_optional_maintenance[i]] - 9 in maintenance_optional_list:
                    maintenance_fee += optional_cost[optional_utility_index[ranking_optional_maintenance[i]]] - optional_cost[optional_utility_index[ranking_optional_maintenance[i]] - 9]
                    maintenance_cost_maintenance[optional_utility_index[ranking_optional_maintenance[i]] - 9] += \
                    optional_cost[optional_utility_index[ranking_optional_maintenance[i]]] - optional_cost[
                        optional_utility_index[ranking_optional_maintenance[i]] - 9]
                else:
                    maintenance_fee += optional_cost[optional_utility_index[ranking_optional_maintenance[i]]]
                    if optional_utility_index[ranking_optional_maintenance[i]] <= 14:
                        maintenance_cost_maintenance[optional_utility_index[ranking_optional_maintenance[i]]] += optional_cost[optional_utility_index[ranking_optional_maintenance[i]]]
                    else:
                        maintenance_cost_maintenance[optional_utility_index[ranking_optional_maintenance[i]] - 9] += \
                        optional_cost[optional_utility_index[ranking_optional_maintenance[i]]]
    ...
    maintenance_cost_preventive = preventive_action * P_cost_list
    preventive_cost = np.sum(maintenance_cost_preventive)
    ...
    return maintenance_necessary_list, maintenance_optional_list, maintenance_fee, preventive_cost, inspection_cost, reward, maintenance_cost_maintenance, maintenance_cost_preventive
...
class environment():
    ...
    def step(self, bridge_infor, states, actions, hidden_state):
        ...
        maintenance_necessary_list, maintenance_optional_list, maintenance_fee, preventive_cost, inspection_cost, reward, maintenance_cost_maintenance, maintenance_cost_preventive = Maintenance_list(
            deck_M_utility, superstructure_M_utility, substructure_M_utility,
            deck_M_cost, superstructure_M_cost, substructure_M_cost,
            deck_P_cost, superstructure_P_cost, substructure_P_cost, actions)

        if maintenance_fee < 0:
            print(substructure_M_cost)
            print('column', np.where(column_expected_state >= 3)[0])
            print('column', np.where(column_expected_state > 2)[0])
        ...
        # print('new_budget:',new_budget / 3e5, 'bridge_risk:', bridge_risk, 'action_reward', action_reward, 'bridge_condition',bridge_condition)

        cost = np.array([final_reward, maintenance_fee, preventive_cost, inspection_cost, bridge_risk])
        return new_state, cost, new_hidden_state, maintenance_necessary_list, maintenance_optional_list, observation_value, maintenance_cost_maintenance, maintenance_cost_preventive
...
class Agent():
    ...
    def Actor_learn(self, state, action, TD_error):
        ...
        with tf.GradientTape() as tape:
            ...
            cross_entropy = - tf.multiply(tf.math.log(action_prob + 1e-20), one_action)
            loss_actor = tf.reduce_sum(tf.multiply(cross_entropy, TD_error))
            entropy = tf.reduce_sum(- action_prob * tf.math.log(action_prob + 1e-20))
            loss = loss_actor - 0.002 * entropy
    ...
    def pre_training(self, initial_state, hidden_state, exploration_step):
        """
        :param initial_state: the intial_state is different for different workers
        :param hidden_state:
        :param budget:
        :param exploration_step:
        :return:
        ...
        # memory
        memory_states = np.zeros((exploration_step * 3, 1941))
        memory_actions = np.zeros((exploration_step * 3))
        memory_reward = np.zeros((exploration_step * 3, 4))
        memory_new_states = np.zeros((exploration_step * 3, 1941))
        memory_hidden_state = np.zeros((exploration_step * 3, 196))

        # get MINI_STEP trajectory
        for i in range(exploration_step):
            initial_state_0 = initial_state[0, :, :]
            initial_state_1 = initial_state[1, :, :]
            initial_state_2 = initial_state[2, :, :]

            memory_states[i, :] = initial_state_0.copy()
            memory_states[i + exploration_step, :] = initial_state_1.copy()
            memory_states[i + exploration_step * 2, :] = initial_state_2.copy()

            action_0 = get_action(self.Actor, initial_state_0, greedy=False)
            action_1 = action_necessary.action_necessary_select(initial_state_1, np.argmax(initial_state_1[0, 1841:1941]), booling = 1)
            action_2 = action_necessary.action_necessary_select(initial_state_2, np.argmax(initial_state_2[0, 1841:1941]), booling = 0)
            # worker 0
            New_belief_state_0, reward_0, new_hidden_state_0, _, _ = self.Environment.step(
                Bridge_information, initial_state_0, action_0, hidden_state[0, :])
            # worker 1
            New_belief_state_1, reward_1, new_hidden_state_1, _, _ = self.Environment.step(
                Bridge_information, initial_state_1, action_1, hidden_state[1, :])
            # worker 2
            New_belief_state_2, reward_2, new_hidden_state_2, _, _ = self.Environment.step(
                Bridge_information, initial_state_2, action_2, hidden_state[2, :])


            memory_actions[i] = action_0
            memory_actions[i + exploration_step] = action_1
            memory_actions[i + exploration_step * 2] = action_2
            memory_reward[i, :] = reward_0
            memory_reward[i + exploration_step, :] = reward_1
            memory_reward[i + exploration_step * 2, :] = reward_2

            memory_hidden_state[i, :] = hidden_state[0, :].copy()
            memory_hidden_state[i + exploration_step, :] = hidden_state[1, :].copy()
            memory_hidden_state[i + exploration_step * 2, :] = hidden_state[2, :].copy()

            memory_new_states[i, :] = New_belief_state_0.copy()
            memory_new_states[i + exploration_step, :] = New_belief_state_1.copy()
            memory_new_states[i + exploration_step * 2, :] = New_belief_state_2.copy()

            initial_state = np.array([New_belief_state_0, New_belief_state_1, New_belief_state_2])
            hidden_state = np.array([new_hidden_state_0, new_hidden_state_1, new_hidden_state_2])

        New_belief_state = initial_state.copy()
        new_hidden_state = hidden_state.copy()
        reward_sum = np.sum(memory_reward[0:20, 0])

        return (memory_states, memory_actions, memory_reward, memory_new_states, memory_hidden_state,
                New_belief_state, reward_sum, new_hidden_state)
...
def main():
    ...
    for i in range(0, num_episode):
        t = 0
        states = np.array([initial_states, initial_states, initial_states])
        hidden_state = np.array([initial_hidden_state, initial_hidden_state, initial_hidden_state])
        reward_sum = 0
        ...
        while t < max_over_step:
            ...
            memory_states, memory_actions, memory_reward, memory_new_states, memory_hidden_state, New_belief_state, reward, hidden_state= agent.pre_training(
