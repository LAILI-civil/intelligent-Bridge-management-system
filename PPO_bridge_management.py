...
class Agent():
    ...
    def _compute_gae(self, rewards, values, last_value, gamma=0.99, lam=0.95):
        """
        rewards: (T,)
        values:  (T,)
        last_value: scalar V(s_T)  (你选的模式 1：bootstrap 用 V(s_T))
        """
        T = len(rewards)
    ...
    def collect_rollout(self, initial_state, hidden_state, rollout_len):
        """
        收集一段 rollout（PPO 用）
        return:
          states: (T, state_dim)
        ...
        states = np.zeros((rollout_len, self.input_shape), dtype=np.float32)
        actions = np.zeros((rollout_len,), dtype=np.int32)
        old_logp = np.zeros((rollout_len,), dtype=np.float32)
        rewards = np.zeros((rollout_len,), dtype=np.float32)
        values = np.zeros((rollout_len,), dtype=np.float32)

        cur_state = initial_state.copy()
        cur_hidden = hidden_state.copy()

        for t in range(rollout_len):
            states[t, :] = cur_state.reshape(-1)

            v = float(self.Critic(cur_state).numpy().reshape(-1)[0])
            values[t] = v

            a, logp = get_action(self.Actor, cur_state, greedy=False)
            actions[t] = a
            old_logp[t] = logp

            new_state, cost, new_hidden_state, _, _ = self.Environment.step(
                Bridge_information, cur_state, a, cur_hidden
            )

            # 你的 reward 定义在 cost[0]
            rewards[t] = float(cost[0])

            cur_state = new_state.copy()
            cur_hidden = new_hidden_state.copy()

        next_value = float(self.Critic(cur_state).numpy().reshape(-1)[0])
        return states, actions, old_logp, rewards, values, next_value, cur_state, cur_hidden

    def ppo_update(self, states, actions, old_logp, adv, ret, ppo_epochs=4, batch_size=64):
        # advantage 标准化（PPO 常规做法）
        ...
        for _ in range(ppo_epochs):
            ...
            for start in range(0, N, batch_size):
                ...
                s = tf.convert_to_tensor(states[mb], dtype=tf.float32)
                a = tf.convert_to_tensor(actions[mb], dtype=tf.int32)
                old_lp = tf.convert_to_tensor(old_logp[mb], dtype=tf.float32)
                adv_mb = tf.convert_to_tensor(adv[mb], dtype=tf.float32)
                ret_mb = tf.convert_to_tensor(ret[mb], dtype=tf.float32)

                with tf.GradientTape() as tape_pi, tf.GradientTape() as tape_v:
                    logits = self.Actor(s)  # (B, n_actions)
                    logp_all = tf.nn.log_softmax(logits)  # (B, n_actions)

                    # 取出每个样本对应动作的 logp
                    logp = tf.gather(logp_all, a[:, None], batch_dims=1)[:, 0]  # (B,)

                    ratio = tf.exp(logp - old_lp)
                    clipped_ratio = tf.clip_by_value(ratio, 1.0 - self.clip_ratio, 1.0 + self.clip_ratio)

                    # PPO clipped objective（最大化 => 最小化负号）
                    obj1 = ratio * adv_mb
                    obj2 = clipped_ratio * adv_mb
                    loss_pi = -tf.reduce_mean(tf.minimum(obj1, obj2))

                    # entropy bonus
                    probs = tf.nn.softmax(logits)
                    entropy = -tf.reduce_mean(tf.reduce_sum(probs * logp_all, axis=1))
                    loss_pi_total = loss_pi - self.entropy_coef * entropy

                    # value loss
                    v = self.Critic(s)[:, 0]
                    loss_v = tf.reduce_mean(tf.square(ret_mb - v))

                grads_pi = tape_pi.gradient(loss_pi_total, self.Actor.trainable_variables)
                grads_v = tape_v.gradient(loss_v, self.Critic.trainable_variables)
                ...
                self.Critic_optimizer.apply_gradients(zip(grads_v, self.Critic.trainable_variables))
...
def main():
    ...
    # support, 8 component, 5 preventive years
    ...
    rollout_len = max_over_step  # 每个 episode 收集 100 步
    ppo_epochs = 5               # 用同一批 rollout 数据更新 5 次
    batch_size = 100             # 让每个 ppo_epoch 只有 1 个 minibatch（梯度更新次数=ppo_epochs）
    gae_lam = 0.95
    ...
    if not os.path.exists(csv_path):
        with open(csv_path, "w") as f:
            f.write("episode,cumulative_reward\n")

    # Checkpoint (window-best)
    ...
    for i in range(num_episode):
        ...
        states_roll, actions_roll, old_logp_roll, rewards_roll, values_roll, next_value, _, _ = agent.collect_rollout(
            states, hidden_state, rollout_len
        )

        adv, ret = agent._compute_gae(rewards_roll, values_roll, next_value, gamma=gamma, lam=gae_lam)
        agent.ppo_update(states_roll, actions_roll, old_logp_roll, adv, ret,
                         ppo_epochs=ppo_epochs, batch_size=batch_size)

        reward_sum = float(np.sum(rewards_roll))
        cur_window_rewards.append(reward_sum)

        # 输出每轮 cumulative reward
        print("epoch num:", i, " time stemp:", max_over_step, "   Reward:", reward_sum)

        # 写 CSV（保持你原来的频率）
        if i % 100 == 0:
            with open(csv_path, "a") as f:
                f.write(f"{i},{reward_sum}\n")
        ...
        if i % 100 == 0:
            plt.savefig(os.path.join(PLOT_DIR, f"reward_ep_{i}.png"), dpi=150, bbox_inches="tight")

        # 每 5000 个 episode 做一次窗口对比：当前窗口均值 > 上一窗口均值 => 保存模型
        if (i + 1) % window_size == 0:
            cur_window_avg = float(np.mean(cur_window_rewards))
            if prev_window_avg is None:
                print(f"[WINDOW] ep {i - window_size + 1}~{i}: avg={cur_window_avg:.6f} (no previous window)")
            else:
                print(f"[WINDOW] ep {i - window_size + 1}~{i}: avg={cur_window_avg:.6f} vs prev_avg={prev_window_avg:.6f}")
                if cur_window_avg > prev_window_avg:
                    agent.save(ckpt_dir)  # 覆盖保存，始终保留“最近一次提升窗口均值”的模型
                    print(f"[CKPT] Saved window-best model to {ckpt_dir} (avg improved)")

            prev_window_avg = cur_window_avg
            cur_window_rewards = []  # 清空，进入下一个窗口
